 ...
 */
bool Imu::begin(adafruit_bno055_opmode_t mode) {
 ...
 *           REMAP_SIGN_P7]
 */
void Imu::setAxisSign(adafruit_bno055_axis_remap_sign_t remapsign) {
  adafruit_bno055_opmode_t modeback = _mode;
 ...
 */
void Imu::setExtCrystalUse(boolean usextal) {
  adafruit_bno055_opmode_t modeback = _mode;
 ...
 *           system error info
 */
void Imu::getSystemStatus(uint8_t *system_status,
 ...
 */
void Imu::getCalibration(uint8_t *sys, uint8_t *gyro,
...
imu::Vector<3> Imu::getEuler() {
  return getVector(VECTOR_EULER);
}

imu::Vector<3> Imu::getMag() {
  return getVector(VECTOR_MAGNETOMETER);
}
 ...
 */
imu::Vector<3> Imu::getVector(adafruit_vector_type_t vector_type) {
  imu::Vector<3> xyz;
  uint8_t buffer[6];
  memset(buffer, 0, 6);
 ...
 */
bool Imu::getEvent(sensors_event_t *event) {
  /* Clear the event */
  ...
  imu::Vector<3> euler = getVector(Imu::VECTOR_EULER);
 ...
 */
bool Imu::write8(adafruit_bno055_reg_t reg, byte value) {
 ...
 */
byte Imu::read8(adafruit_bno055_reg_t reg) {
  byte value = 0;
 ...
 */
bool Imu::readLen(adafruit_bno055_reg_t reg, byte *buffer,
                              uint8_t len) {
  _wire->beginTransmission(_address);
#if ARDUINO >= 100
  _wire->write((uint8_t)reg);
